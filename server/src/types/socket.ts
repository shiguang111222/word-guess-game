import type { GamePhase, GuessEntry, PlayerPublic, GameOverData } from './game.js';
...
export interface ServerToClientEvents {
  ...
  'game:round-end': (data: { round: number; guesses: GuessEntry[]; eliminated: Array<{ playerId: string; playerName: string; word: string }> }) => void;
  'game:word-hint': (data: { message: string }) => void;
  'game:guess': (data: GuessEntry) => void;
  'game:eliminated': (data: {
    playerId: string;
    playerName: string;
    guesserId: string;
    guesserName: string;
